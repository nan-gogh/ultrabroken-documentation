...
credits: ["Ock", "Kilovictor", "mulberry"]
date: "2023-12-12"
...
## Summary
Holding and then destroying a material a couple of frames before Link culls causes it to get put into a state of constantly respawning when holding another item, allowing for massive duping. It is widely known to be the fastest material duplication glitch in the game.
...
=== "Mineru Cull BID"
	---
	versions: ["1.0.0", "1.1.1", "1.1.2", "1.2.0", "1.2.1", "1.3.0/1.4.0", "1.4.1", "1.4.2", "1.4.3", "Switch 2"]
	obsolete: false
	---
...
=== "Portacull BID"
	...
	1. Create portacull shield
	2. With recall already selected as the current rune, press L to open recall and then pause
	3. Hold a bird egg
	4. Unpause while spamming dpad left to open the shield quick menu exactly one frame later
	5. Drop portacull shield and spam dpad left again to swap to a different shield without unpausing the game
...
3. From now on, every time the currently held item stack is updated by unholding or holding additional items, the items from before the stack was updated will appear at the location of the dropped bundle
...
- Using a purgatory DI equipment stuck in the thrown state, you can stand in the hitbox of the weapon while holding the breakable item to instantly break it during BID setups, which removes the randomness of inconsistent cull durations associated with some BID setups.
...
??? example "Examples & Tutorials (click to expand)"
	...
	- [SDC Cull BID Walkthrough (Old)](https://youtu.be/jQkN32W59Kc)
	- [Updated SDC Cull BID Tutorial](https://youtu.be/xeD2SfNF7zA?si=Qogo-yyiQtAnwS5g&t=276)
	- [Mineru BID Walkthrough](https://youtu.be/8cHT4KoQ_0Y?si=2KsuGYN5e2HWm9CI)
