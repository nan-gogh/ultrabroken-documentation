/**
 * Social Card Copy / Preview
 * Appends a card button to the .ub-page-actions row
 * (created by the content.html template override).
 * Click = open the card in the media previewer overlay.
...
(function () {
  ...
  var _mediaBase = 'https://ultrabroken-media.gl1tchcr4vt.workers.dev/';
  ...
  function init() {
    ...
    if (!uid) return;
    var cardKey = 'social/' + uid + '.png';
    var cardUrl = _mediaBase + cardKey;

    var btn = document.createElement('button');
    btn.className = 'md-content__button md-icon ub-card-copy';
    btn.type = 'button';
    btn.title = 'Preview social card (also copies URL)';
    btn.setAttribute('aria-label', 'Preview social card');
